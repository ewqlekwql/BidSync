...
function allCheck(_allCheckBox) {
    ...
    const productCheckBoxList = document.getElementsByClassName("wishlist-one-product");
...
function allCheckCancel() {
    let checkCancel = document.getElementById("wishlist-Checkreset");
...
//장바구니 담기 버튼을 눌렀을 때 "장바구니에 상품이 담겼습니다"라는 배너가 뜸
let ShoppingBasket = document.querySelectorAll(".wishlist-button1");

ShoppingBasket.forEach(function(button) {
...
function deleteProduct() {
    ...
    let deleteButtons = document.querySelectorAll(".wishlist-button2");

    // 각 삭제 버튼에 이벤트 리스너를 추가합니다.
    deleteButtons.forEach(function (deleteButton) {
        deleteButton.addEventListener('click', function () {
            // 해당 버튼에 연결된 상품의 체크박스를 찾습니다.
            let productElement = deleteButton.closest(".product-Subject"); // 버튼의 부모 .subject 요소 찾기
